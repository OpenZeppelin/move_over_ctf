(function () {
  ...
  function buildResolutionContext(tables) {
    ...
    const fieldHandleTable = tableByKind(tables, "FIELD_HANDLES");
    if (fieldHandleTable && Array.isArray(fieldHandleTable.entries)) {
      for (const entry of fieldHandleTable.entries) {
        const fieldMap = entry && entry.fieldMap && typeof entry.fieldMap === "object" ? entry.fieldMap : Object.create(null);
        ctx.fieldHandles[entry.index] = {
          ownerIdx: toNumberMaybe(fieldMap.owner_idx),
          fieldIdx: toNumberMaybe(fieldMap.field_idx),
        };
      }
    }
  ...
  function buildBackendExecutionModel(functions, tables, ctx, moduleBlob) {
    ...
    if (Array.isArray(ctx.fieldHandles)) {
      for (let i = 0; i < ctx.fieldHandles.length; i += 1) {
        const item = ctx.fieldHandles[i];
        if (!item) {
          model.fieldHandles[i] = null;
          continue;
        }
        const ownerIdx = toNumberMaybe(item.ownerIdx);
        const fieldIdx = toNumberMaybe(item.fieldIdx);
        model.fieldHandles[i] = {
          ownerIdx: Number.isInteger(ownerIdx) ? ownerIdx : null,
          fieldIdx: Number.isInteger(fieldIdx) ? fieldIdx : null,
        };
      }
    }
  ...
  function remapModelWithOffsets(segmentModel, offsets) {
    ...
    if (Array.isArray(model.fieldHandles)) {
      for (const fh of model.fieldHandles) {
        if (!fh || fh.ownerIdx === null || fh.ownerIdx === undefined) continue;
        if (Number.isInteger(fh.ownerIdx)) {
          fh.ownerIdx += offsets.structDefOffset || 0;
        }
      }
    }
  ...
  function mergeExecutionModels(base, segment) {
    ...
    pushAll("fieldHandles", "fieldHandles");
...
`;
  ...
  async function runLevel(payload) {
    ...
    const logTail = Array.isArray(state.logs) ? state.logs.slice(-50).join("\n") : "";
    const writeRefLogs = Array.isArray(state.logs)
      ? state.logs.filter((line) => /WRITE_REF|synthetic|could not get struct/i.test(String(line))).join("\n")
      : "";
    ...
    if (vmError) {
      ...
      return {
        ...
        output: [
          ...
          writeRefLogs ? `WRITE_REF/synthetic logs:\n${writeRefLogs}` : "",
          !runtimeInfo && logTail ? `Recent logs (last 50):\n${logTail}` : "",
        ]
          .filter(Boolean)
          .join("\n\n"),
      };
